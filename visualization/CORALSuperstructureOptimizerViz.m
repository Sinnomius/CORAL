classdef CORALSuperstructureOptimizerViz < handle
    % Visualization patch version: 2026-08-20-v2
    ...
    properties
        ...
        Seed = []
        RecordReefHistory = false
        RecordEventHistory = false
    end
    ...
    methods
        function obj = CORALSuperstructureOptimizerViz(modelFcn, nBinary, xBounds, varargin)
            ...
            addParameter(p,'RecordReefHistory',obj.RecordReefHistory);
            addParameter(p,'RecordEventHistory',obj.RecordEventHistory);
            parse(p,varargin{:});
        ...
        function result = optimize(obj,varargin)
            ...
            if obj.RecordReefHistory
                reefHistory{1} = obj.captureReefSnapshot(reef,occupied,0);
            end

            if obj.RecordEventHistory
                eventHistory{1} = obj.emptyEventSnapshot(0);
            end
            ...
            for it = 1:maxIterations
                % Initialize per-iteration visualization/event record.
                currentEvents = obj.emptyEventSnapshot(it);
                ...
                for ell = 1:nLarvae
                    ...
                    [reef,occupied,settled,cellIndex,replaced] = obj.settleLarva(larva,reef,occupied);
                    if settled
                        obj.OperatorSuccess(op)=obj.OperatorSuccess(op)+1;
                        if obj.RecordEventHistory
                            currentEvents.settlementCells(end+1)=cellIndex;
                            currentEvents.settlementOperators(end+1)=op;
                            currentEvents.replacedResident(end+1)=replaced;
                        end
                    end
                end
                ...
                for k = 1:min(nBuds,numel(ranked))
                    ...
                    [reef,occupied,budSettled,budCell,budReplaced] = obj.settleLarva(bud,reef,occupied);
                    if obj.RecordEventHistory && budSettled
                        currentEvents.buddingCells(end+1)=budCell;
                        currentEvents.buddingReplaced(end+1)=budReplaced;
                    end
                end

                [reef,occupied,depredatedCells] = obj.depredate(reef,occupied);
                if obj.RecordEventHistory
                    currentEvents.depredationCells = depredatedCells;
                end

                if obj.populationDiversity(reef,occupied) < obj.BleachingThreshold
                    [reef,occupied,bleachedCells,recolonizedCells] = obj.bleachAndRecover(reef,occupied);
                    if obj.RecordEventHistory
                        currentEvents.bleachingTriggered = true;
                        currentEvents.bleachedCells = bleachedCells;
                        currentEvents.recolonizedCells = recolonizedCells;
                    end
                end
                ...
                if obj.RecordReefHistory
                    reefHistory{it+1} = obj.captureReefSnapshot(reef,occupied,it);
                end

                if obj.RecordEventHistory
                    eventHistory{it+1} = currentEvents;
                end
            ...
            if obj.RecordReefHistory
                result.reefHistory = reefHistory(1:it+1);
            else
                result.reefHistory = {};
            end

            if obj.RecordEventHistory
                result.eventHistory = eventHistory(1:it+1);
            else
                result.eventHistory = {};
            end
        end
    ...
    methods (Access=private)
        ...
        function [reef,occupied,settled,cellIndex,replaced]=settleLarva(obj,larva,reef,occupied)
            settled=false;
            cellIndex=NaN;
            replaced=false;
        ...
        function [reef,occupied,depredatedCells]=depredate(obj,reef,occupied)
            depredatedCells=[];
            occ=find(occupied);
            ...
            for k=1:numel(worst)
                if rand<obj.DepredationProbability
                    ...
                    depredatedCells(end+1)=worst(k); %#ok<AGROW>
                end
            end
        end
        ...
        function [reef,occupied,bleachedCells,recolonizedCells]=bleachAndRecover(obj,reef,occupied)
            bleachedCells=[];
            recolonizedCells=[];

            occ=find(occupied);
            if numel(occ)<4
                before=find(occupied);
                [reef,occupied]=obj.recolonize(reef,occupied,4);
                after=find(occupied);
                recolonizedCells=setdiff(after,before)';
                return;
            end

            ranked=obj.rankCorals(reef,occ);
            startIdx=max(2,round(numel(ranked)/2));
            pool=ranked(startIdx:end);
        ...
        function snap=captureReefSnapshot(obj,reef,occupied,iteration)
            snap.iteration=iteration;
            snap.occupied=occupied(:);
            snap.y=nan(obj.ReefSize,obj.NBinary);
            snap.f=nan(obj.ReefSize,1);
            snap.v=nan(obj.ReefSize,1);
            snap.feasible=false(obj.ReefSize,1);

            for ii=1:obj.ReefSize
                if occupied(ii)
                    snap.y(ii,:)=reef(ii).y;
                    snap.f(ii)=reef(ii).f;
                    snap.v(ii)=reef(ii).v;
                    snap.feasible(ii)=reef(ii).feasible;
                end
            end
        end

        function ev=emptyEventSnapshot(~,iteration)
            ev.iteration=iteration;
            ev.settlementCells=[];
            ev.settlementOperators=[];
            ev.replacedResident=[];
            ev.buddingCells=[];
            ev.buddingReplaced=[];
            ev.depredationCells=[];
            ev.bleachingTriggered=false;
            ev.bleachedCells=[];
            ev.recolonizedCells=[];
        end
